...
def clean_df_nulls(df):
    return df.na.drop(subset=["home_team_goal", "away_team_goal", "home_team_api_id", "away_team_api_id"])
...
def transform_match_df(match_df_raw: DataFrame) -> DataFrame:
    ...
    reduced_match_df = reduced_match_df.withColumn(
        "result",
        func.when(
            reduced_match_df.home_team_goal.cast(IntegerType()) > reduced_match_df.away_team_goal.cast(IntegerType()),
            func.lit("WIN")
        ).when(
            reduced_match_df.home_team_goal.cast(IntegerType()) == reduced_match_df.away_team_goal.cast(IntegerType()),
            func.lit("TIE")
